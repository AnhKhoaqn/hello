d3.json("/home/data/").then(function(response) {
    ...
    let categoryOrdersByMonth = new Map();
    ...
    response.forEach(row => {
        ...
        if (maNhomHang && tenNhomHang && maDonHang && ngayThangNam) {
            let categoryKey = `[${maNhomHang}] ${tenNhomHang}`;

            if (!categoryOrdersByMonth.has(ngayThangNam)) {
                categoryOrdersByMonth.set(ngayThangNam, new Map());
            }
            if (!categoryOrdersByMonth.get(ngayThangNam).has(categoryKey)) {
                categoryOrdersByMonth.get(ngayThangNam).set(categoryKey, new Set());
            }
            categoryOrdersByMonth.get(ngayThangNam).get(categoryKey).add(maDonHang);
    ...
    categoryOrdersByMonth.forEach((categoriesMap, monthKey) => {
        let totalSLDon = totalOrdersByMonth.get(monthKey).size || 1;
        categoriesMap.forEach((orders, categoryName) => {
            if (!groupedData.has(categoryName)) {
                groupedData.set(categoryName, []);
            }
            groupedData.get(categoryName).push({
                month: monthKey,
    ...
    console.log("Dữ liệu vẽ biểu đồ:", formattedData); 

    let colorScale = d3.scaleOrdinal(d3.schemeTableau10).domain(formattedData.map(d => d.name));

    drawLineChart(formattedData, colorScale, "#chart");
});
